...
class CommitLogCheckpointStrategy {
  ...
  public CommitLogCheckpoint computeCheckpoint() {
    ...
    return CommitLogCheckpoint.create(
        checkpointTime, computeBucketCheckpointTimes(firstPassTimes, threshold));
  }

  /**
   * Returns a map from all bucket IDs to their current last written time values, fetched without a
   * transaction so with no guarantee of consistency across buckets.
   */
  @VisibleForTesting
  ImmutableMap<Integer, DateTime> readBucketTimestamps() {
    ...
    return ofy.doWithFreshSessionCache(
        () ->
            CommitLogBucket.loadAllBuckets().stream()
                .collect(
   ...
   * where "new" means having a commit time after the per-bucket timestamp in the given map. When no
   * such commit logs exist, the threshold value is set to END_OF_TIME.
   */
  @VisibleForTesting
  ...
  ImmutableMap<Integer, DateTime> computeBucketCheckpointTimes(
      ImmutableMap<Integer, DateTime> firstPassTimes, final DateTime threshold) {
