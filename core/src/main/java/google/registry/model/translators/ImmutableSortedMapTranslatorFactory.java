 ...
 *
 * <p>This translator serves a similar purpose to {@link
 * com.googlecode.objectify.impl.translate.MapifyTranslatorFactory @Mapify}. Except this maintains
 * perfect immutability of the field value. Subclasses may override the {@link
 * #transformBeforeSave(ImmutableSortedMap)} methods to perform mutation on a per-concrete type
 * basis. This abstraction is also more readable than {@code @Mapify} because it shifts the
 * boilerplate into translator magic, rather than convoluting model data structures.
 *
 * <h3>Entity Data Layout</h3>
 *
 * <p>For example, if you had an {@code ImmutableSortedMap<String, String>} on a field named {@code
 * field}, then this would look like:
 *
 * <pre>{@code
 * field.key: key1 -> key2
 ...
 * }</pre>
 *
 * <p>If you had an {@code ImmutableSortedMap<String, EmbeddedClass>} on a field named {@code
 * field}, where {@code EmbeddedClass} defines two {@code foo} and {@code bar} fields, then the
 * embedded properties might look like:
 *
 * <pre>{@code
 * field.key: key1 -> key2
...
abstract class ImmutableSortedMapTranslatorFactory<K extends Comparable<? super K>, V>
  ...
  @Override
  public final Translator<ImmutableSortedMap<K, V>> create(
      ...
      return new ListNodeTranslator<ImmutableSortedMap<K, V>>() {
        ...
        protected ImmutableSortedMap<K, V> loadList(Node node, LoadContext ctx) {
          ...
          for (Node child : node) {
            try {
              map.put(
                  keyTranslator.load(child.get(keyProperty), ctx),
        ...
        protected Node saveList(
          ...
          ImmutableSortedMap<K, V> mapToSave =
              transformBeforeSave(ImmutableSortedMap.copyOfSorted(nullToEmpty(mapFromPojo)));
