 ...
 * offset) in decimal, the middle columns are each digit in hexadecimal, and the column on the right
 * is the ASCII representation where non-printable characters are represented by {@code '.'}.
 *
...
public final class HexDumper extends OutputStream {
  ...
  @Nonnegative public static final int DEFAULT_PER_LINE = 16;

  @Nonnegative public static final int DEFAULT_PER_GROUP = 4;

  private Writer upstream;

  @Nonnegative private final int perLine;

  @Nonnegative private final int perGroup;

  private long totalBytes;

  @Nonnegative private int lineCount;
  ...
  /**
   * Calls {@link #dumpHex(byte[], int, int)} with {@code perLine} set to {@value #DEFAULT_PER_LINE}
   * and {@code perGroup} set to {@value #DEFAULT_PER_GROUP}.
   */
  ...
  /**
   * Calls {@link #HexDumper(Writer, int, int)} with {@code perLine} set to {@value
   * #DEFAULT_PER_LINE} and {@code perGroup} set to {@value #DEFAULT_PER_GROUP}.
   */
   ...
   * This is done to avoid system call overhead {@code out} is a resource, and reduces the chance of
   * lines being broken by other threads writing to {@code out} (or its underlying resource) at the
   * same time.
   *
   ...
   *
   * <p>If you attempt to write to this object after calling this method, {@link
   * IllegalStateException} will be thrown. However, it's safe to call close multiple times, as
   * subsequent calls will be treated as a no-op.
   *
   * @throws IOException upon failure to write to upstream {@link Writer#write(String) writer}.
   */
  @Override
